...
class Network(nn.Module):
    def __init__(self, seq_len, pred_len, patch_len, stride, padding_patch, d_model=None):
        ...
        self.fc8 = nn.Linear(pred_len, d_model)
    ...
    def forward(self, s, t):
        ...
        # 综合趋势-季节特征
        seasonality_out = torch.reshape(s, (B, C, self.pred_len))
        trend_out = torch.reshape(t, (B, C, self.pred_len))
        time_feature = seasonality_out + trend_out
        time_feature = self.fc8(time_feature)  # 线性输出：BCP -> BCD [batch_size,var_nums,d_model]
        return time_feature
